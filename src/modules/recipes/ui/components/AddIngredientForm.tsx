...
import { useAddRecipeLine } from '../../data/recipes'
...
export function AddIngredientForm({ orgId, recipeId, canWrite }: AddIngredientFormProps) {
  const [productId, setProductId] = useState<string>('')
  const [qty, setQty] = useState<number>(0)
  const [unit, setUnit] = useState<'kg' | 'ud'>('ud')

  const products = useProducts(orgId)
  const addLine = useAddRecipeLine(recipeId, orgId)
