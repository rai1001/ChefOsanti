...
export function useWeekEvents(hotelId?: string, weekStart?: string) {
  return useQuery({
    ...
    enabled: Boolean(hotelId && isValidUuid(hotelId ?? '') && weekStart),
  })
}
...
export function useOrdersSummary(orgId?: string, weekStart?: string) {
  return useQuery({
    ...
    enabled: Boolean(orgId && isValidUuid(orgId ?? '') && weekStart),
  })
}
...
export function useDashboardPurchaseMetrics(orgId?: string, hotelId?: string, day?: string) {
  return useQuery({
    ...
    enabled: Boolean(orgId && isValidUuid(orgId ?? '') && hotelId && isValidUuid(hotelId ?? '') && day),
  })
}
...
export function useDashboardRollingGrid(orgId?: string, hotelId?: string, rangeStart?: string) {
  return useQuery({
    ...
    enabled: Boolean(orgId && isValidUuid(orgId ?? '') && hotelId && isValidUuid(hotelId ?? '') && rangeStart),
  })
}
...
export function useDashboardNote(orgId?: string, userId?: string, weekStart?: string) {
  ...
  const query = useQuery({
    ...
    enabled: Boolean(orgId && isValidUuid(orgId ?? '') && userId && isValidUuid(userId ?? '') && weekStart),
  })
...
export function useStaffAvailability(hotelId?: string, date?: string) {
  return useQuery({
    ...
    enabled: Boolean(hotelId && isValidUuid(hotelId ?? '') && date),
  })
}
