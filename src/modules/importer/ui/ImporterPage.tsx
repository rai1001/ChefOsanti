            ...
            if (file.name.endsWith('.csv')) {
                ...
                if (entity === 'events') {
                    ...
                    if (activeParsedData) {
                        ...
                        // Fallback re-parse or use current validation state? 
                        // Actually if we are here, handleFileChange should have set state. 
                        // But if file was just set and user clicked fast? 
                        // Safest is to rely on state if present, or re-parse.
                        // For now assume state is set.
                        if (sheetNames.length > 0 && selectedSheetName) {
                            rows = sheets[selectedSheetName]
                        } else {
                            // Only happens if state lost
